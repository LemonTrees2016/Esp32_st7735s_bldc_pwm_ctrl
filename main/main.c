...
extern uint32_t g_switch1_flag;
...
void NumTest11(void *pvParameters)
{
	// set font file
	FontxFile fx16[2];
	FontxFile fx24[2];
	...
	lcdDrawLine(&dev, 96, 0, 96, 160, BLUE);
	lcdDrawLine(&dev, 96, 32, 128, 32, BLUE);
	lcdDrawLine(&dev, 96, 64, 128, 64, BLUE);
	lcdDrawLine(&dev, 96, 96, 128, 96, BLUE);
	lcdDrawLine(&dev, 96, 128, 128, 128, BLUE);
	lcdDrawLine(&dev, 96, 160, 128, 160, BLUE);
	
	lcdDrawFillRect(&dev, 96, 0, 128, 32, GRAY);
	lcdDrawFillRect(&dev, 96, 32, 128, 64, PURPLE);
	lcdDrawFillRect(&dev, 96, 64, 128, 96, GRAY);
	lcdDrawFillRect(&dev, 96, 96, 128, 128, PURPLE);
	lcdDrawFillRect(&dev, 96, 128, 128, 160, GRAY);

	
	lcdDrawFillRect(&dev, 0, 0, 96, 160, YELLOW);
	...
	while (1) {
		...
		if (g_ReadFlg) {
			...
			if (g_switch_flag % 2) {
				lcdDrawFillRect(&dev, 51, 0, 86, 160, YELLOW);
				sprintf((char*)ascii, "%-3d", g_Cnt1);
				lcdDrawString(&dev, fx32, 51, 0, ascii, color);
			} else {
				lcdDrawFillRect(&dev, 15, 0, 50, 160, YELLOW);
				sprintf((char*)ascii, "%-3d", g_Cnt);
				lcdDrawString(&dev, fx32, 15, 0, ascii, color);
			}
...
void NumTest(void *pvParameters)
{
	// set font file
	FontxFile fx16[2];
	FontxFile fx24[2];
...
void app_main(void)
{
	...
	//xTaskCreate(tft, "tftCut", 4096, NULL, 2, NULL);
	
	xTaskCreate(NumTest11, "tftCut", 4096, NULL, 2, NULL);
	ESP_LOGI(TAG, "start switch\n");
	app_main_gpio();
	McPwmCtrl_AppMain();
}
